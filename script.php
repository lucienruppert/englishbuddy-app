<script>
  ...
  function startPrintLevel(level) {
    <?php if ($userObject && $userObject['status'] != 1) { ?>
      ...
      window.open('printViewSent.php?wordLevel=' + level, 'Szófajok', 'fullscreen=yes,toolbar=no,status=no,menubar=no,resizable=yes,location=no,scrollbars=yes');
    <?php } else if ($userObject['status'] == 1) { ?>
      alert(premiumUserOnlyMessage);
    <?php } else { ?>
      alert(notLoggedInMessage);
  ...
  function startPrintMumus() {
    <?php if ($userObject && $userObject['status'] != 1) { ?>
      window.open('printViewSent.php?pkg=mumus&source=szo', 'Szófajok', 'fullscreen=yes,toolbar=no,status=no,menubar=no,resizable=yes,location=no,scrollbars=yes');
    <?php } else if ($userObject['status'] == 1) { ?>
      alert(premiumUserOnlyMessage);
    <?php } else { ?>
      alert(notLoggedInMessage);
