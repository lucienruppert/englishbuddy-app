...
function getWelcomePageText()
{
    $langExt = getLangExtByLangId($GLOBALS['userObject']['forras_nyelv']);
    $query = "select txt_{$langExt} as txt from text_messages where tipus = 'welcomePageText'";
    $result = mysql_query($query);
    if(!$result){
        print mysql_error();
        exit("Nem sikerült: " . $query);
    }
    list($msg) = mysql_fetch_row($result);
    return $msg;
}

function setWelcomePageText($txt)
{
    $langExt = getLangExtByLangId($GLOBALS['userObject']['forras_nyelv']);
    $txt = aposztrofRepToDb($txt);

    $query = "update text_messages set txt_{$langExt} = '{$txt}' where tipus = 'welcomePageText'";
    $result = mysql_query($query);
    if(!$result){
        print mysql_error();
        exit("Nem sikerült: " . $query);
